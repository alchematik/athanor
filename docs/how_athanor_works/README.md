...
the state described in a blueprint with reality.
...
produce a [dependency graph](https://en.wikipedia.org/wiki/Dependency_graph) of resources. 


## Evaluator

The evaluator is up next in the pipeline. The purpose of the evaluator is to traverse the dependency graph created by
the interpreter and evaluate each resource. Athanor makes two evaluation passes.
The first pass constructs a graph of resources in their ideal state by using a stubbed API that 
returns the resource as-is. The second pass uses the provider plugins to fetch the actual state that each resource is in.
...
The differ takes the results of the evaluator and compares the desired state of the components to the actual state they are in and produces a [diff](../core_concepts/diffs.md).
...
The reconciler reconciles the difference in states as indicated by the diff. Based on the diff, the reconciler will leverage the 
provider plugins to create, update, or delete resources. The reconciler reconciles each resource in order of dependency.
